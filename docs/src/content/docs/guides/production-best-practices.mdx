...
description: Security, performance, and deployment guidelines for USwap applications
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

Building production-ready applications with USwap requires careful attention to security, performance, and user experience. This guide covers essential best practices.
...
await uSwap.connectTrezor([Chain.Bitcoin]);
```
...
```typescript
// @noErrorValidation
import { createUSwap, USwapError } from "@tcswap/sdk";

const uSwap = createUSwap();

function validateSwapInput(params: any): void {
  if (!uSwap.validateAddress(params.recipient, params.chain)) {
...
async function getToolboxForChain(chain: Chain) {
  switch (chain) {
    case Chain.Ethereum:
      const { getEvmToolbox } = await import("@tcswap/sdk");
      return getEvmToolbox(chain);

    case Chain.Bitcoin:
      const { getUtxoToolbox } = await import("@tcswap/sdk");
      return getUtxoToolbox(chain);

    default:
      const { getToolbox } = await import("@tcswap/sdk");
...
const avaxBalance = await uSwap.getBalance(Chain.Avalanche);

const chains = [Chain.Ethereum, Chain.Bitcoin, Chain.Avalanche];
const balances = await Promise.all(
  chains.map((chain) => uSwap.getBalance(chain))
);
```
...
async function executeSwap(route: SwapRoute) {
  try {
    const txHash = await uSwap.swap({ route });
...
```typescript
// @noErrorValidation
import { USwapError, USwapApi } from "@tcswap/sdk";
...
const quote = await withRetry(() => USwapApi.getSwapQuote(params));
```
...
async function performSwap(params: SwapParams) {
  ...
  try {
    ...
    const quote = await USwapApi.getSwapQuote(params);

    setState({ status: "approving", message: "Approving tokens..." });
    await uSwap.approveAssetValue(assetValue);

    setState({ status: "swapping", message: "Executing swap..." });
    const txHash = await uSwap.swap({ route: quote.routes[0] });
...
```typescript
// @noErrorValidation
import { Chain, USwapApi } from "@tcswap/sdk";
...
async function trackTransaction(
  ...
  while (attempts < maxAttempts) {
    try {
      const status = await USwapApi.getTrackerDetails({ txHash, chain });
...
async function getOptimalGasPrice(
  ...
  const gasRates = await USwapApi.getGasRate(chain);
...
const txHash = await tracker.track("swap", () => uSwap.swap({ route }));
```
...
```typescript
// @noErrorValidation
import { USwapApi, Chain } from "@tcswap/sdk";

async function healthCheck(): Promise<HealthStatus> {
  const checks = await Promise.allSettled([
    USwapApi.getPrice("ETH"),
    uSwap.validateAddress("0x...", Chain.Ethereum),
