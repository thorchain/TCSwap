...
description: Advanced USwap features for building sophisticated DeFi applications including multi-chain operations, custom plugins, and performance optimization.
---

This guide covers advanced USwap features for building sophisticated DeFi applications.

## Multi-Chain Operations

USwap excels at coordinating operations across multiple blockchains simultaneously.
...
const balances = await Promise.all(
  chains.map((chain) => uSwap.getBalance(chain, true))
);
...
```typescript
// @noErrorValidation
import { AssetValue, FeeOption } from "@tcswap/sdk";
...
const mintQuote = await USwapApi.getSwapQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "10000000",
  buyAsset: "BTC/BTC",
  sourceAddress: uSwap.getAddress(Chain.Bitcoin),
  destinationAddress: uSwap.getAddress(Chain.THORChain),
  slippage: 3,
});

const mintTx = await uSwap.swap({
...
const synthSwapQuote = await USwapApi.getSwapQuote({
  sellAsset: "BTC/BTC",
  sellAmount: "10000000",
  buyAsset: "ETH/ETH",
  sourceAddress: uSwap.getAddress(Chain.THORChain),
  destinationAddress: uSwap.getAddress(Chain.THORChain),
  slippage: 1,
});
...
const redeemQuote = await USwapApi.getSwapQuote({
  sellAsset: "ETH/ETH",
  sellAmount: "1000000000000000000",
  buyAsset: "ETH.ETH",
  sourceAddress: uSwap.getAddress(Chain.THORChain),
  destinationAddress: uSwap.getAddress(Chain.Ethereum),
  slippage: 3,
});

const redeemTx = await uSwap.swap({
...
```typescript
// @noErrorValidation
import { getMemoForDeposit, getMemoForWithdraw, MemoType } from "@tcswap/sdk";
...
await uSwap.swap({ route: modifiedRoute });
```
...
```typescript
import { Chain, ChainId } from '@tcswap/sdk';
import { USwapApi } from '@tcswap/helpers/api';

const gasRatesArray = await USwapApi.getGasRate();
...
```typescript
// @noErrorValidation
import { AssetValue, ProviderName } from '@tcswap/sdk';
import { USwapApi } from '@tcswap/helpers/api';
...
```typescript
import { ChainId } from '@tcswap/types';
import { USwapApi } from '@tcswap/helpers/api';
...
class TransactionTracker {
  ...
  async trackTransaction(txHash: string, chain: Chain) {
    ...
    const interval = setInterval(async () => {
      try {
        const status = await USwapApi.getTrackerDetails({ hash: txHash, chainId: ChainId[chain] });
...
const retry = new USwapRetry();

const result = await retry.executeWithRetry(async () => {
  return await uSwap.swap({ route });
});
```
