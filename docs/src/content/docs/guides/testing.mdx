...
---
title: Testing
description: Comprehensive guide to testing USwap applications with unit tests, integration tests, mocking strategies, and chain-specific examples.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Testing is essential for building reliable USwap applications. This guide covers unit testing, integration testing, mocking strategies, and testing different chain interactions.

## Testing Setup

### Test Framework Configuration

USwap uses Bun's built-in test runner. Set up your testing environment:
...
beforeAll(() => {
  USwapConfig.set({
    apiKeys: {
      uSwap: process.env.TEST_API_KEY || "test-api-key",
    },
    envs: {
      isDev: true,
    },
...
describe('USwap Configuration', () => {
  ...
  test('should set and get API keys', () => {
    ...
    USwapConfig.setApiKey('uSwap', apiKey);

    const config = USwapConfig.get('apiKeys');
    expect(config.uSwap).toBe(apiKey);
  });

  test('should set explorer URLs', () => {
    const customUrl = 'https:

    USwapConfig.setExplorerUrl(Chain.Ethereum, customUrl);

    const explorerUrls = USwapConfig.get('explorerUrls');
  ...
  test('should preserve existing config when adding new keys', () => {
    ...
    USwapConfig.setApiKey('walletConnectProjectId', 'key2');

    const apiKeys = USwapConfig.get('apiKeys');
    expect(apiKeys.uSwap).toBe('key1');
...
describe("USwapError", () => {
  test("should create error with string key", () => {
    const error = new USwapError("core_wallet_connection_not_found");

    expect(error).toBeInstanceOf(USwapError);
    expect(error.name).toBe("USwapError");
  ...
  test("should preserve cause", () => {
    ...
    const uSwapError = new USwapError("api_v2_server_error", originalError);

    expect(uSwapError.cause).toBe(originalError);
  });

  test("should have correct error codes", () => {
    expect(USwapError.ErrorCode.core_wallet_connection_not_found).toBe(10101);
    expect(USwapError.ErrorCode.wallet_ledger_device_not_found).toBe(20104);
    expect(USwapError.ErrorCode.api_v2_server_error).toBe(60002);
  });
});
```
...
import { describe, expect, test } from "bun:test";
import { validateAddress, isValidAssetIdentifier } from "@uswap/helpers";
import { Chain } from "@uswap/types";
...
describe("Wallet Integration", () => {
  ...
  test(
    ...
    async () => {
      ...
      await uSwap.connectKeystore(chains, process.env.TEST_PHRASE);
  ...
  test(
    ...
    async () => {
      ...
      await uSwap.connectKeystore([Chain.Ethereum], process.env.TEST_PHRASE);

      const wallet = await uSwap.getWalletWithBalance(Chain.Ethereum);
  ...
  test("should handle connection errors gracefully", async () => {
    ...
    await expect(
      uSwap.connectKeystore([Chain.Ethereum], invalidPhrase)
    ).rejects.toThrow();
  });
});
```
...
describe("API Integration", () => {
  ...
  test(
    ...
    async () => {
      ...
      const quote = await USwapApi.getSwapQuote(quoteParams);
  ...
  test("should handle invalid asset gracefully", async () => {
    await expect(USwapApi.getPrice("INVALID.ASSET")).rejects.toThrow();
  });

  test(
    "should get gas rates",
    async () => {
      const gasRate = await USwapApi.getGasRate("ETH");
...
import { keystoreWallet } from "@uswap/wallets/keystore";
import { Chain, AssetValue } from "@uswap/helpers";
...
describe("Swap Integration", () => {
  ...
  test(
    ...
    async () => {
      ...
      const quote = await uSwap.getQuote(quoteParams);
  ...
  test("should validate swap parameters", async () => {
    ...
    await expect(uSwap.getQuote(invalidParams as any)).rejects.toThrow();
  });
  ...
  test(
    ...
    async () => {
      ...
      await expect(uSwap.getQuote(quoteParams)).rejects.toThrow();
    },
    { timeout: 30000 }
  );
});
...
import { describe, expect, test } from "bun:test";
import { getUtxoToolbox } from "@uswap/toolboxes/utxo";
import { Chain } from "@uswap/types";
...
export function expectUSwapError(error: any, expectedKey: string) {
  expect(error).toBeInstanceOf(USwapError);
  ...
  test(
    ...
    async () => {
      ...
      const prices = await Promise.all(
        assets.map((asset) => USwapApi.getPrice(asset))
      );
