...
---
title: Creating a USwap Plugin
description: Learn how to create a custom plugin for USwap
---

USwap plugins extend the functionality of the core library to support various DeFi protocols and services. This guide explains how to create a custom plugin for USwap.

## Plugin Structure

A USwap plugin consists of the following components:

1. A plugin factory created with `createPlugin`
2. Plugin methods that interact with external services or smart contracts
3. Integration with the USwap client
...
export const ExamplePlugin = createPlugin({
  name: "example",
  properties: {
    supportedUSwapProviders: ["your-provider-name"],
  },
  methods: ({ getWallet }) => ({
...
export const ExchangePlugin = createPlugin({
  name: "exchange",
  properties: {
    supportedUSwapProviders: [ProviderName.CUSTOM_EXCHANGE],
  },
  methods: ({ getWallet }) => ({
...
## Using Your Plugin with USwap

Once you've created your plugin, you can use it with the USwap client:

```ts twoslash
import { createUSwap, Chain, keystoreWallet } from "@tcswap/sdk";
...
async function connectAndUsePlugin() {
  ...
  const quoteResponse = await uSwap.examplePlugin.getQuote({
    sellAsset: "ETH.ETH",
  ...
  if (quoteResponse.success) {
    const txHash = await uSwap.swap({ route: quoteResponse.data.routes[0] });
  }
}
```
...
When creating a USwap plugin, follow these best practices:
...
By following this guide, you should be able to create, test, and integrate custom plugins with USwap.
