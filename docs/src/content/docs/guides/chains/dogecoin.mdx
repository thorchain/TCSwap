...
description: Complete guide to integrating Dogecoin with USwap SDK for UTXO operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers Dogecoin integration with USwap, including wallet connections, UTXO management, fast block times, and cross-chain swaps.

## Overview

Dogecoin is a fun, friendly cryptocurrency originally created as a joke but has become widely adopted for tipping and micropayments. USwap provides comprehensive Dogecoin support through:
...
const dogeWallet = await uSwap.getWallet(Chain.Dogecoin);
...
```typescript
// @noErrorValidation
import { getDogecoinToolbox } from '@tcswap/toolboxes/utxo';
...
await uSwap.connectKeystore([Chain.Dogecoin], "your mnemonic phrase");

await uSwap.connectLedger([Chain.Dogecoin]);

await uSwap.connectTrezor([Chain.Dogecoin]);
```
...
const dogeAddress = uSwap.getAddress(Chain.Dogecoin);
...
const dogeToBtcQuote = await uSwap.getQuote({
  sellAsset: "DOGE.DOGE",
  sellAmount: "10000",
  buyAsset: "BTC.BTC",
  senderAddress: uSwap.getAddress(Chain.Dogecoin),
...
const dogeToUsdcQuote = await uSwap.getQuote({
  sellAsset: "DOGE.DOGE",
  sellAmount: "5000",
...
const dogeToRuneQuote = await uSwap.getQuote({
  sellAsset: "DOGE.DOGE",
  sellAmount: "1000",
  buyAsset: "THOR.RUNE",
  senderAddress: uSwap.getAddress(Chain.Dogecoin),
...
USwapConfig.setRpcUrl(Chain.Dogecoin, [
  "https:
  "https:
  "https:
]);
...
USwapConfig.setRpcUrl(Chain.Dogecoin, customDogeRpc);
...
USwapConfig.setRpcUrl(Chain.Dogecoin, "https:
...
```typescript
// @noErrorValidation
import { validateDogecoinAddress } from "@tcswap/toolboxes/utxo";
