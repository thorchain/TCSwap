...
description: Complete guide to integrating Avalanche C-Chain with USwap SDK for fast DeFi operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers Avalanche C-Chain integration with USwap, including wallet connections, token transfers, cross-chain swaps, and Avalanche-specific DeFi protocols.

## Overview

Avalanche is a high-performance blockchain platform with sub-second finality and low transaction costs. USwap provides comprehensive Avalanche support through:
...
const avaxWallet = await uSwap.getWallet(Chain.Avalanche);
...
```typescript
// @noErrorValidation
import { getAvalancheToolbox } from '@tcswap/toolboxes/evm';
...
await uSwap.connectKeystore([Chain.Avalanche], "your mnemonic phrase");

await uSwap.connectBrowserWallet(Chain.Avalanche);

await uSwap.connectWalletConnect([Chain.Avalanche]);

await uSwap.connectLedger([Chain.Avalanche]);
```
...
const wAvaxTransfer = await uSwap.transfer({
  recipient: "0x742c4B4F3e0b5b069F5DCF8A65Eaf8d3E888a3E7",
  assetValue: AssetValue.from({
    chain: Chain.Avalanche,
    symbol: "WAVAX",
...
const bridgeQuote = await uSwap.getQuote({
  ...
  recipientAddress: uSwap.getAddress(Chain.Avalanche),
});
...
const ethToAvaxBridge = await uSwap.getQuote({
  sellAsset: "ETH.ETH",
  sellAmount: "0.5",
  buyAsset: "AVAX.AVAX",
  senderAddress: uSwap.getAddress(Chain.Ethereum),
...
const avaxToBtcQuote = await uSwap.getQuote({
  sellAsset: "AVAX.AVAX",
  sellAmount: "10",
  buyAsset: "BTC.BTC",
  senderAddress: uSwap.getAddress(Chain.Avalanche),
...
const pngToRuneQuote = await uSwap.getQuote({
...
const pangolinQuote = await uSwap.getQuote({
...
const getPortfolioBalances = async () => {
  const balances = {};
  const address = uSwap.getAddress(Chain.Avalanche);
...
const pangolinStaking = await toolbox.call({
  ...
  abi: miniChefABI,
  funcName: "deposit",
  funcParams: [1, "5000000000000000000", uSwap.getAddress(Chain.Avalanche)],
});
```
...
USwapConfig.setRpcUrl(Chain.Avalanche, [
  "https:
  "https:
  "https:
]);
...
const joeV2Trade = await toolbox.call({
  ...
  funcParams: [
    ...
    uSwap.getAddress(Chain.Avalanche),
    Date.now() + 600000,
  ],
  txOverrides: {
...
2. **RPC endpoint issues**:
   ...
   USwapConfig.setRpcUrl(Chain.Avalanche, providers);
   ```
