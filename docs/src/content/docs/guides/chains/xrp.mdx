...
description: Complete guide to integrating XRP Ledger with USwap SDK for XRP operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers XRP Ledger integration with USwap, including wallet connections, XRP transfers, trustlines, and cross-chain swaps.

## Overview

XRP Ledger is a decentralized blockchain focused on fast, low-cost payments and remittances. USwap provides comprehensive XRP Ledger support through:
...
const xrpWallet = await uSwap.getWallet(Chain.Ripple);
...
```typescript
// @noErrorValidation
import { getXRPToolbox } from '@tcswap/toolboxes/ripple';
...
await uSwap.connectKeystore([Chain.Ripple], "your mnemonic phrase");

await uSwap.connectXumm([Chain.Ripple]);

await uSwap.connectLedger([Chain.Ripple]);
```
...
const xrpAddress = uSwap.getAddress(Chain.Ripple);

console.log("XRP address:", xrpAddress);

import { validateXRPAddress } from "@tcswap/toolboxes/ripple";
...
const finishEscrow = async (offerSequence: number) => {
  const finishTx = await xrpToolbox.finishEscrow({
    owner: uSwap.getAddress(Chain.Ripple),
    offerSequence,
  });
...
const xrpToBtcQuote = await uSwap.getQuote({
  sellAsset: "XRP.XRP",
  sellAmount: "1000",
  buyAsset: "BTC.BTC",
  senderAddress: uSwap.getAddress(Chain.Ripple),
...
const xrpToUsdcQuote = await uSwap.getQuote({
  sellAsset: "XRP.XRP",
  sellAmount: "500",
...
const multiSignTransaction = async (txBlob: string) => {
  ...
  return {
    txBlob,
    signature,
    account: uSwap.getAddress(Chain.Ripple),
  };
};
...
USwapConfig.setRpcUrl(Chain.Ripple, [
  "wss:
  "wss:
  "wss:
]);
...
USwapConfig.setRpcUrl(Chain.Ripple, customServer);
...
USwapConfig.setRpcUrl(Chain.Ripple, "wss:
