...
description: Complete guide to integrating Ethereum with USwap SDK for DeFi operations and cross-chain swaps
---

import { Card, CardGrid, Code, Tabs, TabItem } from "@astrojs/starlight/components";

This guide covers Ethereum integration with USwap, including wallet connections, token transfers, cross-chain swaps, and EVM-specific features.

## Overview

Ethereum is the leading smart contract blockchain and the foundation of DeFi. USwap provides comprehensive Ethereum support through:
...
const ethWallet = await uSwap.getWallet(Chain.Ethereum);
...
```typescript
// @noErrorValidation
import { getEthereumToolbox } from '@tcswap/toolboxes/evm';
...
await uSwap.connectKeystore([Chain.Ethereum], "your mnemonic phrase");

await uSwap.connectBrowserWallet(Chain.Ethereum);
...
const ethToBtcQuote = await uSwap.getQuote({
  sellAsset: "ETH.ETH",
  sellAmount: "1",
  buyAsset: "BTC.BTC",
...
const swapResult = await uSwap.swap({
...
USwap supports major Ethereum DEXs through integrated routing:
...
const oneinchQuote = await uSwap.getQuote({
...
```typescript
// @noErrorValidation
import { validateEthereumAddress } from "@tcswap/toolboxes/evm";
...
const customTx = await uSwap.transfer({
  recipient: "0x742c4B4F3e0b5b069F5DCF8A65Eaf8d3E888a3E7",
...
USwapConfig.setRpcUrl(Chain.Ethereum, [
  "https:
  "https:
  "https:
]);
...
**New to USwap?** Start with [Getting Started](/start/getting-started) to learn the basics.
