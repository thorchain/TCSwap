...
description: Complete guide to integrating Bitcoin with USwap SDK for UTXO operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers Bitcoin integration with USwap, including wallet connections, UTXO management, cross-chain swaps, and Bitcoin-specific operations.

## Overview

Bitcoin is the first and most valuable cryptocurrency, using a UTXO (Unspent Transaction Output) model. USwap provides comprehensive Bitcoin support through:
...
const btcWallet = await uSwap.getWallet(Chain.Bitcoin);
...
```typescript
// @noErrorValidation
import { getBitcoinToolbox } from '@tcswap/toolboxes/utxo';
...
await uSwap.connectKeystore([Chain.Bitcoin], "your mnemonic phrase");
...
const addresses = {
  nativeSegwit: uSwap.getAddress(Chain.Bitcoin),
...
const btcToEthQuote = await uSwap.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.1",
  buyAsset: "ETH.ETH",
...
const btcToUsdcQuote = await uSwap.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.05",
...
const btcToRuneQuote = await uSwap.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.01",
  buyAsset: "THOR.RUNE",
  senderAddress: uSwap.getAddress(Chain.Bitcoin),
...
USwapConfig.setRpcUrl(Chain.Bitcoin, [
  "https:
  "https:
  "https:
]);
...
USwapConfig.setRpcUrl(Chain.Bitcoin, customBitcoinRpc);
...
USwapConfig.setRpcUrl(Chain.Bitcoin, "https:
...
3. **Validate Addresses**:
   ...
   ```typescript
   import { validateBitcoinAddress } from "@tcswap/toolboxes/utxo";
