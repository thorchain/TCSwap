...
description: Complete guide to integrating Keplr wallet with USwap SDK v4 for Cosmos ecosystem support
---

Keplr is the leading browser extension wallet for the Cosmos ecosystem, providing secure access to Cosmos-based networks. USwap SDK v4 offers comprehensive support for Keplr, enabling seamless integration with Cosmos, THORChain, and other IBC-enabled chains.

## Supported Chains

USwap supports Keplr for these Cosmos ecosystem chains:
...
```ts twoslash
import { USwap, keplrWallet, Chain, WalletOption } from "@tcswap/sdk";
...
async function connectKeplr() {
  try {
    ...
    const balance = await uSwap.getBalance(Chain.Cosmos, true);
...
async function connectMultipleCosmosChains() {
  ...
  try {
    await uSwap.connectKeplr(cosmosChains);
...
async function connectLeapWallet() {
  try {
    await uSwap.connectKeplr([Chain.Cosmos], WalletOption.LEAP);
...
import { USwap, keplrWallet, Chain, AssetValue } from "@tcswap/sdk";
...
async function sendAtom() {
  ...
  try {
    ...
    const txHash = await uSwap.transfer({
      assetValue: amount,
      recipient,
      memo: "Keplr transaction via USwap",
    });
...
await uSwap.connectKeplr([Chain.THORChain]);
...
await uSwap.connectKeplr([Chain.Cosmos, Chain.Noble]);
...
await uSwap.connectKeplr([Chain.Noble]);
...
import { USwap, keplrWallet, Chain } from "@tcswap/sdk";
...
async function addCustomCosmosChain() {
  ...
  try {
    ...
    if (error.message.includes("chain_not_supported")) {
      ...
      console.log("USwap will prompt Keplr to add the chain");
    }
    throw error;
  }
}
...
async function transactionWithCustomGas() {
  ...
  try {
    const wallet = await uSwap.getWalletWithBalance(Chain.Cosmos);
...
async function verifyAddress() {
  const uSwap = USwap({ wallets: { ...keplrWallet } });
  await uSwap.connectKeplr([Chain.Cosmos]);
...
import { USwap, keplrWallet, Chain, WalletOption, AssetValue } from "@tcswap/sdk";
...
export function KeplrWallet({ onConnect, onError }: KeplrWalletProps) {
  ...
  const connectKeplr = async () => {
    ...
    try {
      await uSwap.connectKeplr(selectedChains);
...
This comprehensive guide covers all aspects of Keplr wallet integration with USwap SDK v4, including Cosmos ecosystem support, IBC transfers, and advanced features specific to Cosmos-based chains.
