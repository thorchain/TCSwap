...
description: Complete guide to integrating Coinbase Wallet with USwap SDK v4 for mobile and desktop support
---

Coinbase Wallet is a popular self-custody wallet that provides access to Ethereum and other EVM-compatible networks. USwap SDK v4 supports both the mobile Coinbase Wallet app (via WalletConnect/SDK) and the browser extension, offering seamless integration for your dApp.

## Supported Chains

USwap supports Coinbase Wallet for these EVM-compatible chains:
...
import { USwap, coinbaseWallet, Chain } from "@tcswap/sdk";
...
async function connectCoinbaseWallet() {
  try {
    ...
    await uSwap.connectCoinbaseWallet([Chain.Ethereum]);
...
async function connectMultipleChains() {
  ...
  try {
    ...
    await uSwap.connectCoinbaseWallet(supportedChains);
...
import { USwap, coinbaseWallet, Chain, AssetValue } from "@tcswap/sdk";
...
await uSwap.connectCoinbaseWallet([Chain.Base]);
...
await uSwap.connectCoinbaseWallet([Chain.Ethereum, Chain.Polygon, Chain.Arbitrum]);
...
async function manageNetworks() {
  ...
  try {
    ...
    await uSwap.connectCoinbaseWallet(allChains);
...
import { USwap, coinbaseWallet, evmWallet, Chain, WalletOption } from "@tcswap/sdk";
...
async function connectOptimalCoinbaseWallet() {
  ...
  try {
    if (preferredMethod === 'extension' && hasCoinbaseExtension) {
      ...
      const evmUSwap = USwap({ wallets: { ...evmWallet } });
      await evmUSwap.connectEVMWallet([Chain.Ethereum], WalletOption.COINBASE_WEB);

      console.log("✅ Connected via Coinbase Browser Extension");
      return evmUSwap;
    } else {

      const sdkUSwap = USwap({
        config: {
          integrations: {
            coinbase: {
      ...
      await sdkUSwap.connectCoinbaseWallet([Chain.Ethereum]);
...
export function CoinbaseWallet({ onConnect, onError }: CoinbaseWalletProps) {
  ...
  const connectCoinbase = async () => {
    ...
    try {
      await uSwap.connectCoinbaseWallet(selectedChains);
      const walletAddress = uSwap.getAddress(selectedChains[0]);
...
This comprehensive guide covers all aspects of Coinbase Wallet integration with USwap SDK v4, including both mobile SDK and browser extension support, multi-chain functionality, and security best practices.
