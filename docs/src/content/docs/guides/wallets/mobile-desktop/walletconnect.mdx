...
description: Complete guide to integrating WalletConnect with USwap SDK v4 for mobile and desktop wallet support
---

WalletConnect is a protocol that enables seamless connection between dApps and mobile wallets through QR code scanning or deep linking. USwap SDK v4 provides comprehensive WalletConnect v2 support across multiple blockchain networks, enabling users to connect their favorite mobile wallets to your application.

## Supported Chains

USwap supports WalletConnect for these chains:
...
```ts twoslash
import { USwap, walletconnectWallet, Chain } from "@tcswap/sdk";
...
async function connectWalletConnect() {
  try {
    ...
    await uSwap.connectWalletconnect([Chain.Ethereum]);
...
async function connectMultipleChains() {
  ...
  try {
    await uSwap.connectWalletconnect(supportedChains);
...
async function connectWithAdvancedOptions() {
  ...
  try {
    await uSwap.connectWalletconnect([Chain.Ethereum], walletconnectOptions);
...
import { USwap, walletconnectWallet, Chain, AssetValue } from "@tcswap/sdk";
...
await uSwap.connectWalletconnect([Chain.Ethereum, Chain.Polygon, Chain.Arbitrum]);
...
await uSwap.connectWalletconnect([Chain.Cosmos]);
...
await uSwap.connectWalletconnect([Chain.THORChain]);
...
async function connectEVMChains() {
  ...
  try {
    await uSwap.connectWalletconnect(evmChains);
    ...
    for (const chain of evmChains) {
      try {
        const balance = await uSwap.getBalance(chain, false);
...
async function connectCosmosEcosystem() {
  ...
  try {
    await uSwap.connectWalletconnect(cosmosChains);
...
export function WalletConnect({ onConnect, onError }: WalletConnectProps) {
  ...
  const connectWalletConnect = async () => {
    ...
    try {
      await uSwap.connectWalletconnect(selectedChains);
...
This comprehensive guide covers all aspects of WalletConnect integration with USwap SDK v4, including multi-chain support, mobile wallet connectivity, and security best practices for production applications.
