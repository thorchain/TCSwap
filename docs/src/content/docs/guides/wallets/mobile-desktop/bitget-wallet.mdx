...
description: Complete guide to integrating Bitget Wallet with USwap SDK v4 for multi-chain support
---

Bitget Wallet is a comprehensive multi-chain wallet that supports a wide range of blockchain networks. USwap SDK v4 provides extensive support for Bitget Wallet across all its supported chains, making it an excellent choice for multi-chain applications.

## Supported Chains

USwap supports Bitget Wallet for these chains:
...
```ts twoslash
import { USwap, bitgetWallet, Chain } from "@tcswap/sdk";
...
async function connectBitgetEthereum() {
  try {
    await uSwap.connectBitget([Chain.Ethereum]);
...
async function connectMultipleChains() {
  ...
  try {
    await uSwap.connectBitget(supportedChains);
...
async function connectBitgetBitcoin() {
  try {
    await uSwap.connectBitget([Chain.Bitcoin]);
...
async function connectBitgetSolana() {
  try {
    await uSwap.connectBitget([Chain.Solana]);
...
import { USwap, bitgetWallet, Chain, AssetValue } from "@tcswap/sdk";
...
await uSwap.connectBitget([Chain.Ethereum, Chain.Polygon, Chain.Arbitrum]);
...
await uSwap.connectBitget([Chain.Cosmos]);
...
await uSwap.connectBitget([Chain.Tron]);
...
async function manageEVMChains() {
  ...
  try {
    await uSwap.connectBitget(evmChains);
...
async function getPortfolioOverview() {
  ...
  try {
    await uSwap.connectBitget(allChains);
...
async function handleChainSpecificIssues(chain: Chain) {
  try {
    await uSwap.connectBitget([chain]);
  } catch (error) {
    switch (chain) {
      case Chain.Bitcoin:
...
async function secureTransfer(chain: Chain, recipient: string, amount: number) {
  ...
  const uSwap = USwap({ wallets: { ...bitgetWallet } });
  await uSwap.connectBitget([chain]);
...
export function BitgetWallet({ onConnect, onError }: BitgetWalletProps) {
  ...
  const connectBitget = async () => {
    ...
    try {
      await uSwap.connectBitget(selectedChains);
...
This comprehensive guide covers all aspects of Bitget Wallet integration with USwap SDK v4, including multi-chain support, security best practices, and troubleshooting for the most common issues across different blockchain networks.
