...
description: Complete guide to integrating Zcash transparent addresses with USwap
---

USwap provides comprehensive support for Zcash through the UTXO toolbox with specialized handling for Zcash transparent addresses. This guide covers everything you need to know about working with Zcash in your applications.

## Overview

The Zcash integration in USwap supports:
...
USwap supports Zcash transparent addresses which provide Bitcoin-like functionality on the Zcash network:
...
```ts twoslash
import { AssetValue, Chain, getUtxoToolbox } from "@tcswap/sdk";
...
```ts twoslash
import { Chain, FeeOption, getUtxoToolbox } from "@tcswap/sdk";
...
The Zcash toolbox automatically detects network based on USwap configuration:

```ts twoslash
import { Chain, USwapConfig, getUtxoToolbox } from "@tcswap/sdk";
...
```ts twoslash
import { AssetValue, Chain, USwapError, getUtxoToolbox } from "@tcswap/sdk";
...
1. **Transparent Addresses Only**: USwap does not support Zcash shielded addresses (z-addresses)
...
USwap has evolved to use modern cryptographic libraries for Zcash operations:
