...
description: Comprehensive guide to handling errors in USwap applications
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

Proper error handling is crucial for building robust USwap applications. This guide covers the USwapError system, common error scenarios, debugging techniques, and recovery patterns.

## USwapError System

USwap provides a comprehensive error system with specific error codes for different failure scenarios.

### USwapError Class

The `USwapError` class extends the native `Error` class with additional context:
...
USwap errors are organized by category with specific numeric codes:
...
```typescript
// @noErrorValidation
import { USwapError, AssetValue } from "@tcswap/helpers";
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    if (this.state === "OPEN") {
      if (Date.now() - this.lastFailTime < this.timeout) {
        throw new USwapError("core_chain_halted", {
...
const performSwapWithCircuitBreaker = async (route: SwapRoute) => {
  return circuitBreaker.execute(() => uSwap.swap({ route }));
};
```
...
const getQuoteWithFallback = async (params: QuoteParams) => {
  return withFallback({
    primary: () => uSwapApi.getSwapQuote(params),
    fallbacks: [
...
class ErrorTracker {
  static captureError(error: Error, context: Partial<ErrorContext>) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error("USwap Error:", error);
...
async function debugTransaction(
  ...
  try {
    const toolbox = await uSwap.getToolbox(chain);
...
class ErrorMonitor {
  ...
  logError(error: USwapError, chain?: string) {
...
- **Always check for USwapError**: Use `instanceof USwapError` checks
...
function getUserFriendlyError(error: USwapError) {
  const errorKey = error.message.split(":")[0];
  return (
